...
def main() -> None:
    ...
    if MemoryStore.is_initialized(ROOT):
        ...
        ids = capture_transcript(store, transcript_path, {
            ...
            "cwd": hook_input.get("cwd", ""), "source": source,
        })
    ...
    # Detach the child so it survives if the hook's session group is killed.
    # Windows: CREATE_NO_WINDOW (not DETACHED_PROCESS — breaks Agent SDK I/O).
